...
The main options I looked into were [Sage](http://www.sagemath.org/), [Maxima](http://maxima.sourceforge.net/index.html), and [SymPy](http://www.sympy.org/en/index.html), and I eventually decided to take SymPy for a spin. This will also be my first post in Python, which is well-supported by the knitr / R Markdown framework.
...
$$
\mathbb{E}_X[X^n] = \int_{-\infty}^{\infty}x^nf(x)dx = 1
$$
...
To write SymPy expressions, one first defines the symbols that are manipulated. We start out with $x$, the variable with respect to which PDFs are defined, and $t$, the variable for MGFs. We then define some pretty simple helpful functions for expressing our expectations of interest.
...
summarize(normal)
```
All four quantities are correct! (See [Wikipedia](https://en.wikipedia.org/wiki/Normal_distribution).)
...
The Beta distribution is the first one that SymPy was unable to evaluate. When I tried the mean, variance, and MGF, all the integrals hanged, and I had to abort the operation.^[This is also documented by another user as a [GitHub issue](https://github.com/sympy/sympy/issues/11169).]
