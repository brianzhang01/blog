...
I won't give a full explanation of each these data structures. The idea is similar to prefix sums, in which the full list is decomposed into a set of sublists, and the range query is calculated over each of these sublists. However, when a value of the list is updated, only $O(\log N)$ range queries need to be recalculated, compared to $O(N)$ for prefix sums. To answer a given range query takes $O(\log N)$ time, compared to $O(1)$ for prefix sums.
