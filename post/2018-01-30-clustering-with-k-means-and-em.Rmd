...
```{r k3-base, fig.show="animate", cache=TRUE}
...
```{r k3-fixed-big, fig.show="animate", cache=TRUE}
...
```{r k3-fixed-small, fig.show="animate", cache=TRUE}
sigma001 = NULL
for(k in 1:K) {
...
```{r k8-base, fig.show="animate", cache=TRUE}
# Run K means and EM
K = 8
set.seed(3)
centers = points[sample(1:N, K),]
...
```{r k8-fixed-big, fig.show="animate", cache=TRUE}
fixed_sigma = partial(em.m, fix_sigma=sigma, fix_weights=weights)
em_results = iterate_em(nsteps, K, points, em.e, fixed_sigma, m_params.init)
make_visualization(points, kmeans_results, em_results, nsteps, K)
```
...
```{r k8-fixed-small, fig.show="animate", cache=TRUE}
sigma001 = NULL
for(k in 1:K) {
  sigma001[[k]] = diag(D)*0.01
}
